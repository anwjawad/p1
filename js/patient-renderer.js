...
function renderPatientsGrid(patients) {
    ...
    patients.forEach((p, index) => {
        ...
        if (appData.pharmacyMode) {
            ...
            // --- Labs Badges ---
            let labBadges = '';
            let parsedLabs = p.labs;
            if (typeof parsedLabs === 'string') {
                try { parsedLabs = JSON.parse(parsedLabs); } catch(e) { parsedLabs = {}; }
            }

            if (parsedLabs && typeof parsedLabs === 'object') {
                Object.entries(parsedLabs).forEach(([k, v]) => {
            ...
            let symptomText = '';
            let ecogBadge = ''; // Initialize ECOG Badge

            // Parse symptoms object if it's stored as a string
            let parsedSymptoms = p.symptoms;
            if (typeof parsedSymptoms === 'string') {
                try { parsedSymptoms = JSON.parse(parsedSymptoms); } catch(e) { parsedSymptoms = {}; }
            }

            if (parsedSymptoms) {
                // Extract ECOG
                if (parsedSymptoms['ECOG'] && parsedSymptoms['ECOG'].active && parsedSymptoms['ECOG'].value !== null && parsedSymptoms['ECOG'].value !== undefined) {
                    const ecogValue = parsedSymptoms['ECOG'].value;
                    let ecogColor = 'bg-violet-100 text-violet-700 border-violet-200';
                    if (ecogValue >= 3) ecogColor = 'bg-orange-100 text-orange-700 border-orange-200';
                    
                    ecogBadge = `<div class="px-2 py-0.5 rounded-md border text-[10px] font-bold ${ecogColor} whitespace-nowrap shadow-sm ml-2 flex items-center" title="ECOG Performance Status">ECOG ${ecogValue}</div>`;
                }

                // Filter where value.active is true and ignore ECOG
                const activeSymptoms = Object.entries(parsedSymptoms)
                    .filter(([k, v]) => v && v.active === true && k !== 'ECOG');
