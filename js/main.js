...
function selectWard(wardName) {
    ...
    function renderPatientsGrid(patients) {
        ...
        patients.forEach((p, index) => {
            ...
            // Interactive classes if NOT selection mode (standard hover)
            // If Selection Mode, we handle clicks differently
            if (!appData.selectionMode) {
                classes += " hover:shadow-lg hover:-translate-y-1 cursor-pointer group";
            } else {
                classes += " cursor-pointer"; // Clickable for selection
            }
            ...
            if (appData.selectionMode) {
                ...
                const checkIconClass = isSelected ? '' : 'hidden'; // check icon
                selectionCheckbox = `
            ...
            let symptomText = '';
            // Only show top 2 symptoms to save space
            if (p.symptoms) {
                let count = 0;
                Object.entries(p.symptoms).forEach(([k, v]) => {
                    if (v.active) {
                        ...
                        if (count <= 2) {
                            ...
                            symptomText += `<div class="text-xs text-rose-600 font-medium mb-1 truncate">• ${k} ${note}</div>`;
                        }
                    }
                });
                if (count > 2) symptomText += `<div class="text-[10px] text-slate-400 pl-2">+${count - 2} more</div>`;
            }

            // Plan Preview
            let planPreview = '';
            if (p.plan && Array.isArray(p.plan) && p.plan.length > 0) {
                planPreview = `<div class="mt-2 pt-2 border-t border-slate-50 flex gap-1 overflow-hidden">`;
                p.plan.slice(0, 3).forEach(item => { // Show max 3 icons
                    let icon = 'circle';
                    let color = 'text-slate-400';
                    if (item.type === 'medication') { icon = 'pills'; color = 'text-indigo-500'; }
                    if (item.type === 'equipment') { icon = 'mask-ventilator'; color = 'text-cyan-500'; }
                    if (item.type === 'consult') { icon = 'user-doctor'; color = 'text-purple-500'; }
                    if (item.type === 'note') { icon = 'note-sticky'; color = 'text-amber-500'; }

                    planPreview += `<i class="fa-solid fa-${icon} ${color} text-[10px] bg-slate-50 p-1 rounded-full"></i>`;
                });
                planPreview += `</div>`;
            }

            // Highlight Star (Always visible if highlighted, or on hover)
            ...
            <div class="flex justify-between items-start mb-3 pr-8">
                <div>
                     <h3 class="font-bold text-lg text-slate-800 group-hover:text-medical-600 transition-colors truncate max-w-[180px]">${p.name} <span class="text-xs font-normal text-slate-400 ml-1">(${parseInt(p.age)})</span></h3>
            ...
            <div class="space-y-1 mb-3">
                <div class="text-sm text-slate-600 truncate"><span class="font-medium text-slate-400 text-xs uppercase mr-1">Dx</span> ${p.diagnosis}</div>
            </div>
            ...
            <div class="flex items-center gap-2 mt-3 pt-3 border-t border-slate-50 justify-between">
                <div class="flex items-center gap-2">
                    <span class="text-xs text-slate-500 font-medium truncate max-w-[120px]"><i class="fa-solid fa-user-doctor mr-1"></i> ${p.provider || 'Unassigned'}</span>
                </div>
                <!-- Meds Button: Hide in selection mode to avoid clicks -->
