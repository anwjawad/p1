...
async function fetchData() {
    try {
        ...
        // Cache-busting to ensure latest DB changes are always fetched on refresh
        const fetchUrl = GAS_API_URL + (GAS_API_URL.includes('?') ? '&' : '?') + 'nocache=' + new Date().getTime();
...
function openPlanModal(patient) {
    appData.currentPatient = patient; appData.pristinePatient = JSON.parse(JSON.stringify(patient)); // Ensure context
...
const COMMON_SYMPTOMS = [
    'Pain', 'Nausea', 'Vomiting', 'Dyspnea', 'Shortness of Breath', 'Constipation', 'Diarrhea',
    'Delirium', 'Confusion', 'Disoriented', 'Agitation', 'Fatigue', 'Drowsiness', 'Somnolence', 'Altered level of consciousness',
    'Anorexia', 'Lack of Appetite', 'Anxiety', 'Depression', 'Insomnia', 
    'Cough', 'Dry Mouth', 'Pruritus', 'Dizziness', 'Dysarthria',
    'Bloated', 'Abdominal Distension', 'Odynophagia', 'Dysphagia',
    'DVT', 'Edema', 'Blurred vision', 'Skin discoloration', 'Mucositis',
    'Neuropathy', 'Dysuria', 'Anemia', 'Well-being'
];
...
function renderSymptomsUI() {
    ...
    const activeKeys = Object.keys(tempSymptoms).filter(k => tempSymptoms[k] && tempSymptoms[k].active && k !== 'ECOG');
    ...
    const ecogLabels = {
        ...
        3: '3 - Capable of only limited selfcare; confined to bed or chair > 50% of waking hours',
        4: '4 - Completely disabled; cannot carry on any selfcare; totally confined to bed or chair'
    };
    const currentEcog = (tempSymptoms['ECOG'] && tempSymptoms['ECOG'].active && tempSymptoms['ECOG'].value !== undefined) ? tempSymptoms['ECOG'].value : null;

    [0, 1, 2, 3, 4].forEach(val => {
        const btn = document.getElementById(`ecog-btn-${val}`);
        if (!btn) return;
        if (currentEcog === val) {
            btn.className = "flex-1 py-1.5 rounded-lg border-2 border-violet-500 bg-violet-50 text-sm font-bold text-violet-700 shadow-sm transition-all transform scale-105";
        } else {
            btn.className = "flex-1 py-1.5 rounded-lg border border-slate-200 text-sm font-bold text-slate-600 hover:bg-violet-50 hover:text-violet-600 hover:border-violet-200 transition-all";
        }
    });

    const statusText = document.getElementById('ecog-status-text');
    if (statusText) {
        statusText.innerText = currentEcog !== null ? ecogLabels[currentEcog] : 'Select ECOG Performance Status';
    }
}

function setECOG(val) {
    tempSymptoms['ECOG'] = { active: true, value: val };
    renderSymptomsUI();
}
...
function saveSymptomsData() {
    ...
    // Convert tempSymptoms to a JSON string for the backend to save into a single cell properly.
    // If sent as an object, GAS will forcefully convert it to the string "[object Object]".
    appData.currentPatient.symptoms = JSON.stringify(tempSymptoms);
...
function openLabsModal(patient) {
    if (!patient) return;
    appData.currentPatient = patient; appData.pristinePatient = JSON.parse(JSON.stringify(patient));
...
function renderNewLabsGrid(labs) {
    ...
    // Render custom keys
    const customContainer = document.getElementById('labs-custom-grid');
    if (customContainer) {
        customContainer.innerHTML = '';
        Object.keys(labs).forEach(key => {
            if (!standardKeys.includes(key)) {
                // If it's a legacy plain string key, convert to standard {value: string} logic, but usually it's an object via updateLabValue.
                const valObj = typeof labs[key] === 'object' ? labs[key] : { value: String(labs[key] || '') };
                const val = valObj.value !== undefined ? valObj.value : '';
                
                const div = document.createElement('div');
                div.className = "p-3 rounded-xl border border-slate-200 bg-white flex flex-col items-center justify-between h-24 relative hover:shadow-sm transition-all group animate-entry";
                div.innerHTML = `
                    <button onclick="removeCustomLab('${key}')" class="absolute -top-2 -right-2 bg-red-100 text-red-600 rounded-full w-5 h-5 flex items-center justify-center text-[10px] opacity-0 group-hover:opacity-100 transition-opacity z-10 hover:bg-red-500 hover:text-white" title="Remove Lab test"><i class="fa-solid fa-times"></i></button>
                    <input type="text" value="${key}" class="w-full text-center text-[10px] font-bold text-slate-400 uppercase bg-transparent outline-none border-b border-dashed border-slate-200 mb-1 focus:text-slate-800 focus:border-slate-400" placeholder="TEST NAME" onchange="renameCustomLab('${key}', this.value)">
                    <input type="text" value="${val}" class="w-full text-center bg-transparent text-lg font-bold text-slate-800 outline-none border-b border-transparent focus:border-blue-400" placeholder="VALUE" onchange="updateLabValue('${key}', this.value)">
                `;
...
function addCustomLabRow() {
    if (!appData.currentPatient.labs) appData.currentPatient.labs = {};
    if (typeof appData.currentPatient.labs === 'string') {
        try { appData.currentPatient.labs = JSON.parse(appData.currentPatient.labs); } catch (e) { appData.currentPatient.labs = {}; }
    }
    const newKey = "TEST_" + Math.floor(Math.random() * 1000);
    appData.currentPatient.labs[newKey] = { value: '' };
    renderNewLabsGrid(appData.currentPatient.labs);
}

function renameCustomLab(oldKey, newKeyInput) {
    if (!newKeyInput || !newKeyInput.trim()) return;
    const newKey = newKeyInput.trim().toUpperCase();
    if (oldKey === newKey) return;
    
    appData.currentPatient.labs[newKey] = appData.currentPatient.labs[oldKey];
    delete appData.currentPatient.labs[oldKey];
    renderNewLabsGrid(appData.currentPatient.labs);
}

function removeCustomLab(key) {
    if (appData.currentPatient.labs && appData.currentPatient.labs[key]) {
        delete appData.currentPatient.labs[key];
        renderNewLabsGrid(appData.currentPatient.labs);
    }
}
...
function saveLabsData() {
    if (appData.currentPatient && typeof appData.currentPatient.labs === 'object') {
        appData.currentPatient.labs = JSON.stringify(appData.currentPatient.labs);
    }
    // Update UI immediately for real-time responsiveness
    renderPatientsGrid(appData.wards[appData.currentWard] || appData.patients);
